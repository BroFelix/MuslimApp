import 'package:hijri_calendar/src/calendar/digits_converter.dart';
...
class HijriCalendar {
  ...
  static const Map<String, Map<String, Map<int, String>>> _local = {
    'en': {'long': monthNames, 'short': monthShortNames, 'days': wdNames, 'short_days': shortWdNames},
    'ar': {'long': arMonthNames, 'short': arMonthShortNames, 'days': arWkNames, 'short_days': arShortWdNames},
  };
  ...
  String gregorianToHijri(int pYear, int pMonth, int pDay) {
    ...
    int day = (pDay);
    int month = (pMonth); // -1; // Here we enter the Index of the month (which starts with Zero)
    int year = (pYear);
    ...
    int cjdn = (365.25 * (y + 4716)).floor() + (30.6001 * (m + 1)).floor() + day - jgc - 1524;
  ...
  String toFormat(String format) => this.format(
        hYear,
        hMonth,
        hDay,
        format,
  ...
  String format(year, month, day, format) {
    ...
    if (newFormat.contains("DDDD")) {
      newFormat = newFormat.replaceFirst("DDDD", "${_local[language]!['days']![wkDay ?? weekDay()]}");

      // Fri
    } else if (newFormat.contains("DD")) {
      newFormat = newFormat.replaceFirst("DD", "${_local[language]!['short_days']![wkDay ?? weekDay()]}");
    }
    ...
    if (newFormat.contains("MMMM")) {
      newFormat = newFormat.replaceFirst("MMMM", _local[language]!['long']![month]!);
    } else {
      if (newFormat.contains("MM")) {
        newFormat = newFormat.replaceFirst("MM", _local[language]!['short']![month]!);
      }
    }
